...
% (c) 2004-2016 Dan Borglund <dodde@kth.se> and David Eller <dlr@kth.se>
  ...
  cb = ones(nsup, 1);
  # AT here to derive EI(y) and GK(y)
  ...
  npm = size(dpm,1);
  for ki = 1:npm
    geo = attach_conmass(geo, dpm(ki,1), dpm(ki,2), dpm(ki,3));
  end
  ...
  nk = length(ktable);
  # You need the tail plane aerodynamic effect but without the mass distribution
